      ...
      <div className="max-w-[1400px] mx-auto px-5 sm:px-6 md:px-12 relative z-10 flex flex-col justify-between min-h-[50vh]">
        ...
        <div className="flex flex-col lg:flex-row justify-between gap-12 md:gap-16 lg:gap-8 mb-20 md:mb-24">
          ...
          <div className="lg:w-1/2 flex flex-col sm:flex-row gap-12 sm:gap-16 lg:justify-end">
            ...
            <div className="flex flex-col gap-4 min-w-[200px]">
              ...
              {[
                { label: 'GitHub', href: 'https://github.com/MahateerMuhammad', icon: Github },
                { label: 'Email', href: 'https://mail.google.com/mail/?view=cm&fs=1&to=mahateermuhammad100@gmail.com', icon: Mail },
                { label: 'LinkedIn', href: 'https://www.linkedin.com/in/mahateer-muhammad-a74284356', icon: Linkedin },
              ].map((link) => (
                <a
                  key={link.label}
                  href={link.href}
                ...
                >
                  <div className="flex items-center gap-3">
                    <link.icon size={16} className="text-white/40 group-hover:text-white transition-colors" />
                    <span className="font-mono text-xs md:text-sm font-bold uppercase text-white/80 group-hover:text-white tracking-[0.14em] md:tracking-[0.2em] transition-colors">{link.label}</span>
                  </div>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="group flex items-center justify-between border border-white/10 bg-[#111111] hover:bg-cyan-400 hover:border-cyan-400 transition-colors duration-300 p-3 mt-1"
                title="Open Discord profile"
              >
                <div className="flex items-center gap-3">
                  <DiscordIcon className="text-white/40 group-hover:text-white transition-colors" size={16} />
                  <div className="flex flex-col items-start gap-1">
                    <span className="font-mono text-xs md:text-sm font-bold uppercase tracking-[0.14em] md:tracking-[0.2em] leading-none transition-colors text-white/80 group-hover:text-white">
                      Discord
                    </span>
                    <span className="font-mono text-[9px] lowercase leading-none transition-colors text-white/40 group-hover:text-white/60">
                      @mahateer
                    </span>
                  </div>
                </div>
        ...
        <div className="pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4 md:gap-4">
          ...
          <div className="font-mono text-[10px] md:text-xs text-white/40 uppercase tracking-[0.14em] md:tracking-[0.2em] text-center md:text-right">
            &copy; {new Date().getFullYear()} MAHATEER MUHAMMAD. ALL RIGHTS RESERVED.
          </div>
        </div>
