    ...
    <nav className={`fixed top-0 left-0 w-full p-4 md:p-6 flex justify-between items-center z-50 pointer-events-none transition-all duration-500`}>
      ...
      <div className="lg:hidden pointer-events-auto relative z-50">
        <button
          ...
          className={`w-11 h-11 flex items-center justify-center rounded-full backdrop-blur-md border transition-all duration-300 ${isMenuOpen ? 'bg-black/50 border-white/10 text-white shadow-[0_8px_24px_rgba(0,0,0,0.08)]' : isOnDarkSection ? 'bg-black/25 border-white/25 text-white shadow-[0_8px_24px_rgba(0,0,0,0.22)]' : scrolled ? 'bg-black/50 border-white/10 text-white shadow-[0_8px_24px_rgba(0,0,0,0.08)]' : 'bg-[#0A0A0A]/70 border-white/10 text-white'}`}
        >
      ...
      <GsapPresence>
        {isMenuOpen && (
          <Gsap.div
            ...
            className={`fixed inset-0 z-40 pointer-events-auto lg:hidden backdrop-blur-md overflow-hidden ${isOnDarkSection ? 'bg-black/72' : 'bg-[#111111]/96'}`}
          >
            ...
            <div className="absolute -top-24 -right-16 w-72 h-72 rounded-full bg-cyan-400/15 blur-3xl pointer-events-none" />
            <div className={`absolute -bottom-24 -left-10 w-64 h-64 rounded-full blur-3xl pointer-events-none ${isOnDarkSection ? 'bg-white/12' : 'bg-white/10'}`} />
            <div className={`absolute inset-0 pointer-events-none ${isOnDarkSection ? 'bg-[radial-gradient(circle_at_15%_5%,rgba(255,255,255,0.14),rgba(0,0,0,0)_48%)]' : 'bg-[radial-gradient(circle_at_15%_5%,rgba(255,255,255,0.1),rgba(0,0,0,0)_48%)]'}`} />
            ...
            >
              <div className={`w-full max-w-md mx-auto pb-4 border-b flex items-center justify-between ${isOnDarkSection ? 'border-white/20' : 'border-white/15'}`}>
                <span className={`font-mono text-[10px] uppercase tracking-[0.22em] ${isOnDarkSection ? 'text-white/65' : 'text-white/45'}`}>Navigation Matrix</span>
                <div className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-cyan-400" />
                  <span className={`font-mono text-[10px] tracking-[0.14em] ${isOnDarkSection ? 'text-white/55' : 'text-white/35'}`}>MOBILE</span>
                </div>
              </div>
              ...
              <div className="w-full max-w-md mx-auto mt-6 flex-1 flex flex-col">
                {NAV_ITEMS.map((item, i) => (
                  <Gsap.button
                    ...
                    className={`w-full border-b last:border-b-0 py-4 flex items-center justify-between text-left active:translate-x-0.5 transition-transform ${isOnDarkSection ? 'border-white/20 active:bg-white/5' : 'border-white/15 active:bg-black/5'}`}
                  >
                    <div className="flex items-center gap-4">
                      <span className={`text-[11px] font-mono font-bold tracking-[0.14em] ${isOnDarkSection ? 'text-white/50' : 'text-white/35'}`}>{String(i + 1).padStart(2, '0')}</span>
                      <span className={`text-[30px] leading-none font-black uppercase tracking-tight ${isOnDarkSection ? 'text-white' : 'text-white/90'}`}>{item.label}</span>
                    </div>
                    <span className={`w-8 h-8 rounded-full border flex items-center justify-center ${isOnDarkSection ? 'border-white/20' : 'border-white/15'}`}>
                      <ArrowUpRight size={15} className={isOnDarkSection ? 'text-white/70' : 'text-white/45'} />
                    </span>
                  </Gsap.button>
                ))}
              </div>
              ...
              <Gsap.button
                ...
                className={`w-full max-w-md mx-auto mt-6 h-12 rounded-full text-xs font-bold uppercase tracking-[0.16em] flex items-center justify-center gap-2 shadow-[0_10px_24px_rgba(0,0,0,0.16)] ${isOnDarkSection ? 'bg-white text-black' : 'bg-cyan-400 text-black'}`}
              >
                Let's Talk
                <ArrowUpRight size={14} />
              </Gsap.button>

              <div className={`w-full max-w-md mx-auto mt-5 pt-4 border-t flex items-center justify-between ${isOnDarkSection ? 'border-white/15' : 'border-white/10'}`}>
                <span className={`font-mono text-[10px] tracking-[0.14em] ${isOnDarkSection ? 'text-white/55' : 'text-white/35'}`}>Select Section</span>
                <span className={`font-mono text-[10px] tracking-[0.14em] ${isOnDarkSection ? 'text-white/55' : 'text-white/35'}`}>Tap To Navigate</span>
              </div>
            </Gsap.div>
          </Gsap.div>
        )}
          ...
          {NAV_ITEMS.map((item, index) => (
            <Magnetic key={item.sectionId}>
              <button
                ...
                onClick={() => scrollTo(item.sectionId)}
                className={`relative px-6 py-2.5 rounded-full text-xs font-bold uppercase tracking-[0.12em] md:tracking-[0.16em] transition-colors ${isOnDarkSection ? 'text-white/60 hover:text-white' : 'text-white/60 hover:text-white'}`}
              >
