...
const TechBadge = ({ children }) => (
  <span className="px-3 py-1.5 bg-black/5 font-mono text-[10px] md:text-xs font-bold uppercase tracking-[0.12em] md:tracking-[0.16em] text-white/80">
    {children}
  </span>
);
...
export default function ProjectCaseLayout({
  ...
  return (
    <div className={`bg-[#0A0A0A] text-white font-sans selection:bg-cyan-400 selection:text-white overflow-x-hidden ${mode === 'page' ? 'min-h-screen' : 'h-full flex flex-col'}`}>

      {/* ── Sticky header ─────────────────────── */}
      <div className="sticky top-0 z-40 bg-[#0A0A0A]/90 backdrop-blur-md border-b border-white/5">
        <div className="max-w-6xl mx-auto px-6 md:px-10 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="font-mono text-[10px] uppercase font-bold tracking-[0.12em] md:tracking-[0.16em] text-white/40 flex items-center gap-2">
              {project.category}
            </span>
            <span className="font-mono text-[10px] uppercase font-bold tracking-[0.12em] md:tracking-[0.16em] text-white/30 border-l border-white/20 pl-3">
              {project.year}
            </span>
          </div>
          <button
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <section className="max-w-4xl mx-auto text-center flex flex-col items-center px-6 md:px-10 pt-16 md:pt-20 pb-12">
          ...
          <Gsap.p
            ...
            className="text-base md:text-lg leading-7 md:leading-8 text-white/60 max-w-2xl mx-auto"
          >
            {project.tagline}
          </Gsap.p>
          ...
          >
            {project.links?.live && (
              <a href={project.links.live} target="_blank" rel="noreferrer"
                className="bg-cyan-400 text-white px-8 py-3.5 font-bold uppercase text-xs tracking-wider hover:bg-black hover:text-white transition-all duration-300 rounded-[2px] flex items-center gap-2 shadow-sm">
        ...
        {project.heroImg && (
          <section className="px-6 md:px-10 pb-16">
            <Gsap.div
              ...
              className="max-w-5xl mx-auto border border-white/5 p-2 bg-white rounded-lg shadow-sm"
            >
              <div className="w-full bg-neutral-100/50 rounded-[4px] overflow-hidden aspect-video relative flex items-center justify-center border border-white/5">
                <img
